...
def parse_input(file_path):
	...
	NUMBER_OF_RESERVOIRS = tanks_index - reservoir_index - 2
	NUMBER_OF_TANKS = pipes_index - tanks_index - 2
	NUMBER_OF_PIPES = pumps_index - pipes_index - 2
	NUMBER_OF_PUMPS = valves_index - pumps_index - 2
	NUMBER_OF_VALVES = tags_index - valves_index - 2
...
def parse_output(file_path, NUMBER_OF_ELEMENTS,leaking_node_id):
	...
	for i in xrange(0,NUMBER_OF_NODES):
		...
		current_line = list(current_line.split()[:5])    # 5 in order to include the contamination data
		# current_line[:1] = map(int,current_line[:1])
		current_line[1:5] = map(float,current_line[1:5])
	...
	# result_beginning_index += 7
	result_beginning_index = lines.index('Node Results at 24:00:00 hrs:')
	result_beginning_index += 5

	for i in xrange(0,NUMBER_OF_NODES):
		current_line = lines[result_beginning_index+i]
		current_line = list(current_line.split()[:5])
		# current_line[:1] = map(int,current_line[:1])
		current_line[1:5] = map(float,current_line[1:5])
	...
	node_first_time_store = pd.DataFrame(node_first_time_store,columns = ['id','demand','head','pressure','quality'])
	node_second_time_store = pd.DataFrame(node_second_time_store,columns = ['id','demand','head','pressure','quality'])
...
def isConnected(node_input_df,pipe_input_df,node_first_time_store,node_second_time_store,link_first_time_store,link_second_time_store):
	...
	for i in xrange(0,len(unique_node_id)):
		...
		for j in xrange(0,len(current_node_rows)):
			...
			temp_static_burst_time = temp_distance / temp_velocity
...
def leakMatrixCreation(leaking_node_id,unique_node_id,isConnected_list,connected_distance,connected_velocity,connected_time,static_burst_time,flow,node_first_time_store,node_second_time_store):
	...
	while len(stack) > 0:
		...
		for j in xrange(0,len(isConnected_list[parent_node_index])):
			...
			original_contamination = node_first_time_store['quality'][current_node_index]
			new_contamination = node_second_time_store['quality'][current_node_index]

			# if original_pressure > (1.005 * new_pressure):		#modelling pressure drop as indication of leak affectation
			if new_contamination > 0 :	# modeling contamination presence in new node	
