...
read_triangle_nodes = read.table("./brezo_files/colorado.1.ele")
triangle_nodes = as.matrix(read_triangle_nodes)
colnames(triangle_nodes) = NULL

read_node_coordinates = read.table("./brezo_files/colorado.1.node")
...
read_coordinate_mapping = read.csv("./brezo_files/epanet_colorado_coordinate_mapping.csv",header=TRUE)
...
population_values = as.numeric(population_values)
print(population_values)
print(length(which(is.na(population_values))))
