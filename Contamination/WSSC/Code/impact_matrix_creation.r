...
read_triangle_nodes = read.table("./brezo_files/wssc.1.ele")
triangle_nodes = as.matrix(read_triangle_nodes)
colnames(triangle_nodes) = NULL

read_node_coordinates = read.table("./brezo_files/wssc.1.node")
...
read_coordinate_mapping = read.csv("./brezo_files/epanet_montgomery_coordinate_mapping.csv",header=TRUE)
...
population_values = runif(length(unique_node_id),min(triangle_population_normalized,max(triangle_population_normalized)))
