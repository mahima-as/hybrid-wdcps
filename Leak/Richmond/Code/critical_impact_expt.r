...
# print(pure_static_coverage_locations)
# print(pure_static_impact_locations)
# print(hybrid_coverage_mobile_locations)
# print(hybrid_impact_mobile_locations)

final_impact_score_array = c()

for(i in 1:length(unique_node_id))
{
	current_leak = i
	impact_score_current_leak = computeTriangleImpactProduct(impactMatrix,triangle_score,current_leak)
	temp = cbind(i,impact_score_current_leak)
	final_impact_score_array = rbind(final_impact_score_array,temp)
}

final_impact_score_array = as.matrix(final_impact_score_array)
minval = min(final_impact_score_array[,2])
maxval = max(final_impact_score_array[,2])
indices = which(final_impact_score_array[,2] == 0)
for(i in 1:length(indices))
{
	final_impact_score_array[i,2] = runif(1,minval,maxval)
}
final_impact_score_array = final_impact_score_array[order(-final_impact_score_array[,2]),]
...
for(i in 1:6)
{
	...
	leaking_indices = final_impact_score_array[1:number_of_events,1]
	...
	for(k in 1:length(leaking_indices))
	{
		

		
		current_leak = leaking_indices[k]
...
write.csv(final_df,file="critical.csv",row.names=FALSE)
